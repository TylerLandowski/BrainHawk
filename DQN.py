...
class DQN:
	...
	def build_network(self, model):
		if model == "nvidia":
			...
			qn = Sequential()
			qn.add(BatchNormalization(input_shape = self.input_shape))
			qn.add(Conv2D(16, kernel_size = (5, 5), strides = (1, 1), activation = 'relu'))
			qn.add(MaxPooling2D(pool_size = (2, 2), strides = (2, 2), padding = 'valid'))
			qn.add(Conv2D(32, kernel_size = (3, 3), strides = (1, 1), activation = 'relu'))
			qn.add(MaxPooling2D(pool_size = (2, 2), strides = (2, 2), padding = 'valid'))
			qn.add(Conv2D(64, kernel_size = (3, 3), strides = (1, 1), activation = 'relu'))
			qn.add(Flatten())
			qn.add(Dense(50, activation = 'relu'))
			qn.add(Dense(self.action_count, activation = None))
			self.network = qn
		...
		elif model == "space_invaders":
			si = Sequential()
			si.add(BatchNormalization(input_shape = self.input_shape))
			si.add(Conv2D(filters = 32, kernel_size = (8, 8), strides = (4, 4), padding = "valid", activation = "elu"))
			si.add(Conv2D(filters = 64, kernel_size = (4, 4), strides = (2, 2), padding = "valid", activation = "elu"))
			si.add(Conv2D(filters = 64, kernel_size = (3, 3), strides = (2, 2), padding = "valid", activation = "elu"))
			si.add(Flatten())
			si.add(Dense(self.action_count, activation = None))
			self.network = si
